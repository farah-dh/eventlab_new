...
const totalRevenue   = computed(() => events.value.reduce((s, e) => {
  if (e.price && e.seats_booked) return s + (parseFloat(e.price) * e.seats_booked)
  return s
}, 0))
const fillRate = computed(() => {
  if (!totalSeats.value) return 0
  return Math.round((totalBooked.value / totalSeats.value) * 100)
})
...
const getSeatColor = (pct: number) => {
  if (pct >= 90) return '#EF4444'
  if (pct >= 60) return '#F59E0B'
  return '#10B981'
}
const getSeatBg = (pct: number) => {
  ...
  <div class="od">
    ...
    <div class="od-header">
      ...
      <div class="od-header__right">
        ...
        <button class="od-refresh" @click="fetchData">
      ...
      <div class="od-kpis">
        ...
        <div class="od-kpi od-kpi--emerald">
          ...
          <div class="od-kpi__body">
            ...
            <span class="od-kpi__hint">événements payants</span>
          </div>
      ...
      <div class="od-main-grid">
        ...
        <div class="od-card od-card--full">
          ...
          <div v-else class="od-seats-list">
            <div
              v-for="ev in seatsEvents" :key="ev.id"
              class="od-seat-row"
              :style="`--bar-color: ${getSeatColor(ev.pct)}`"
            >
          ...
          <div class="od-card">
            ...
            <div class="od-card__body">
              <div class="od-donut-wrap">
                <svg viewBox="0 0 120 120" class="od-donut">
                  ...
                  <circle
                    ...
                    stroke="url(#roseGrad)" stroke-width="16"
                    :stroke-dasharray="donutCircumference"
                    :stroke-dashoffset="paidOffset"
                  ...
                  <defs>
                    <linearGradient id="roseGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                      ...
                      <stop offset="100%" stop-color="#D4215A"/>
                    </linearGradient>
                  </defs>
                </svg>
              ...
              <div class="od-donut-legend">
                <div class="od-dl-item">
                  <span class="od-dl-dot" style="background:linear-gradient(135deg,#FF4D7A,#D4215A)"></span>
                  <span class="od-dl-label">Payants</span>
                  <span class="od-dl-val">{{ paidEvents }}</span>
                </div>
                <div class="od-dl-item">
                  <span class="od-dl-dot" style="background:#10B981"></span>
                  <span class="od-dl-label">Gratuits</span>
                  <span class="od-dl-val">{{ freeEvents }}</span>
                </div>
                <div class="od-dl-item">
                  <span class="od-dl-dot" style="background:#F59E0B"></span>
                  <span class="od-dl-label">Actifs</span>
                  <span class="od-dl-val">{{ activeEvents }}</span>
                </div>
                <div class="od-dl-item">
                  <span class="od-dl-dot" style="background:#9CA3AF"></span>
                  <span class="od-dl-label">Inactifs</span>
                  <span class="od-dl-val">{{ inactiveEvents }}</span>
                </div>
                <div class="od-dl-item">
                  <span class="od-dl-dot" style="background:#EF4444"></span>
                  <span class="od-dl-label">Complets</span>
                  <span class="od-dl-val">{{ soldOutEvents }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="od-card">
            ...
            <div class="od-card__body">
              <div v-if="upcomingEvents.length === 0" class="od-empty">
                <span class="od-empty__icon">📅</span>
                ...
                <div class="od-top-item__info">
                  ...
                  <div class="od-top-item__barwrap">
                    <div class="od-top-bar">
                      <div class="od-top-bar__fill" :style="`width:${barWidth(ev.seats_booked||0, ev.seats||0)}%;background:${ev.type===1?'#D4215A':'#10B981'}`"></div>
                    </div>
        ...
        <div class="od-card od-card--full od-card--profile">
          <div class="od-card__head">
            <div class="od-card__head-left">
              <div class="od-card__icon-wrap">👤</div>
              <div>
                <h3 class="od-card__title">Profil organisateur</h3>
                <p class="od-card__sub">Informations de votre compte</p>
              </div>
            </div>
          ...
          <div class="od-profile-grid">
            <div class="od-profile-item">
              <span class="od-profile-label">Organisation</span>
              <span class="od-profile-val">{{ organizer.organization_name || '-' }}</span>
            </div>
            <div class="od-profile-item">
              <span class="od-profile-label">Nom complet</span>
              <span class="od-profile-val">{{ organizer.full_name || `${organizer.firstname || ''} ${organizer.lastname || ''}`.trim() || '-' }}</span>
            </div>
            <div class="od-profile-item">
              <span class="od-profile-label">Email</span>
              <span class="od-profile-val">{{ organizer.email || '-' }}</span>
            </div>
            <div class="od-profile-item">
              <span class="od-profile-label">Téléphone</span>
              <span class="od-profile-val">{{ organizer.dial_code || '' }} {{ organizer.mobile || '-' }}</span>
            </div>
            <div class="od-profile-item">
              <span class="od-profile-label">Pays</span>
              <span class="od-profile-val">{{ organizer.country_code || '-' }}</span>
            </div>
            <div class="od-profile-item">
              <span class="od-profile-label">Solde disponible</span>
              <span class="od-profile-val od-profile-val--rose">{{ parseFloat(organizer.balance || 0).toFixed(3) }} DT</span>
            </div>
            <div class="od-profile-item">
              <span class="od-profile-label">Abonnés</span>
              <span class="od-profile-val">{{ organizer.followers_count || 0 }}</span>
            </div>
            <div class="od-profile-item">
              <span class="od-profile-label">Statut KYC</span>
              <span class="od-profile-val" :style="organizer.kv === 1 ? 'color:#10B981;font-weight:700' : 'color:#F59E0B;font-weight:700'">
                {{ organizer.kv === 1 ? '✅ Vérifié' : '⏳ En attente' }}
              </span>
            </div>
          </div>
        </div>
...
.od-header {
  position: relative;
  background: linear-gradient(135deg, #9B1040 0%, #D4215A 50%, #FF4D7A 100%);
  border-radius: 20px; padding: 28px 32px;
  display: flex; align-items: center; justify-content: space-between;
  overflow: hidden; box-shadow: 0 8px 32px rgba(212,33,90,.3);
...
.od-sub strong { font-weight: 600; color: rgba(255,255,255,.85); }
...
.od-spinner {
  width: 40px; height: 40px; border: 3px solid var(--rose-muted);
...
.od-kpi__bg {
  ...
  width: 110px; height: 110px; border-radius: 50%; opacity: .12;
}
...
.od-kpi__trend {
  ...
  font-size: 12px; font-weight: 800; opacity: .45;
}
...
/* ══ CARD ══ */
.od-card {
  background: var(--white); border: 1px solid #EAECF0;
...
.od-seat-row {
  display: grid; grid-template-columns: 1fr 180px 220px 130px;
...
@media (max-width: 1100px) {
  ...
  .od-row2  { grid-template-columns: 1fr; }
  .od-seat-row { grid-template-columns: 1fr auto; grid-template-rows: auto auto; gap: 10px; }
}
@media (max-width: 700px) {
  .od-kpis { grid-template-columns: 1fr; }
  .od-header { padding: 20px 18px; }
  .od-profile-grid { grid-template-columns: repeat(2, 1fr); }
  .od-seat-row { grid-template-columns: 1fr; }
}
</style>