...
// ── Stripe est sélectionné ? ──────────────────────────────────────────────────
const isStripeGateway = computed(() =>
  selectedGateway.value?.alias?.toLowerCase?.() === 'stripe'
)

const isFormValid = computed(() => {
  // Si Stripe → pas besoin de remplir la carte manuellement
  if (isStripeGateway.value) {
    return (
...
const confirmPayment = async () => {
  ...
  // ══════════════════════════════════════════════════════
  // CAS 1 : STRIPE → rediriger vers Stripe Checkout
  // ══════════════════════════════════════════════════════
  if (isStripeGateway.value) {
    try {
      const res = await apiFetch('/payments/deposits/', {
        method: 'POST',
        body: JSON.stringify({
          method_code: selectedGateway.value.code,
      ...
      if (checkoutUrl) {
        // Sauvegarder les infos du panier pour la page success
        localStorage.setItem('pending_order', JSON.stringify({
          items:    [...cart.items],
          customer: { ...customer.value },
          total:    cart.totalPrice,
          gateway:  selectedGateway.value?.name,
        }))
      ...
      step.value         = 'form'
      apiError.value     = err?.data?.message || err?.message || 'Erreur Stripe. Réessayez.'
    }
    return
  }

  // ══════════════════════════════════════════════════════
  // CAS 2 : PAIEMENT MANUEL (carte bancaire classique)
  // ══════════════════════════════════════════════════════
  ...
  try {
    ...
    try {
      ...
      console.warn('Support ticket failed:', e?.data || e)
    }
  ...
  <div class="ck">
    ...
    <div v-else-if="step === 'processing' || step === 'done'" class="ck-processing">
      ...
      <h2 class="ck-processing__title">
        {{ step === 'done' ? 'Paiement validé !' : isStripeGateway ? 'Redirection vers Stripe…' : 'Traitement en cours…' }}
      </h2>
        ...
        <div class="ck-grid">
          <div class="ck-left">
            ...
            <div v-if="gateways.length > 0" class="ck-section">
              ...
              <div class="ck-section__body">
                ...
                <!-- Message Stripe -->
                <div v-if="isStripeGateway" class="ck-stripe-info">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#6772e5" stroke-width="2">
                    <rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>
                  </svg>
                  <span>Vous serez redirigé vers la page de paiement <strong>Stripe</strong> sécurisée.</span>
                </div>
              </div>
            </div>

            <!-- Carte bancaire (masquée si Stripe) -->
            <div v-if="!isStripeGateway" class="ck-section">
              <div class="ck-section__head">
                <div class="ck-section__icon">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#e8405a" stroke-width="2">
                    <rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/>
              ...
              <div class="ck-section__body">
                <div class="ck-field">
                  ...
                  <div class="ck-input-suffix">
                    <input :value="payment.cardNumber" @input="formatCardNumber" type="text" placeholder="1234 5678 9012 3456" maxlength="19" inputmode="numeric" class="ck-mono" />
                    <span class="ck-suffix-icon">
                      <span v-if="cardType === 'visa'" class="suf-visa">VISA</span>
                      <span v-else-if="cardType === 'mastercard'" class="suf-mc"><span></span><span></span></span>
                      <span v-else-if="cardType === 'amex'" class="suf-amex">AMEX</span>
                      <svg v-else width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#d4bfc7" stroke-width="1.5"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/></svg>
                    </span>
                  </div>
                </div>
                <div class="ck-field">
                  <label>Nom du titulaire <span class="ck-req">*</span></label>
                  <input v-model="payment.cardName" type="text" placeholder="SARRA BEN ALI" style="text-transform:uppercase" />
                </div>
                <div class="ck-row">
                  <div class="ck-field">
                    <label>Date d'expiration <span class="ck-req">*</span></label>
                    <input :value="payment.expiry" @input="formatExpiry" type="text" placeholder="MM/AA" maxlength="5" inputmode="numeric" />
                  </div>
                  <div class="ck-field">
                    <label>CVV <span class="ck-req">*</span> <span class="ck-cvv-tip" title="3 chiffres au dos">?</span></label>
                    <input :value="payment.cvv" @input="formatCvv" type="password" placeholder="•••" maxlength="4" inputmode="numeric" />
                  </div>
                </div>
          ...
          <aside class="ck-right">
            <div class="ck-summary">
              ...
              <div class="ck-items">
                <div v-for="item in cart.items" :key="item.id" class="ck-item">
                  ...
                  <div class="ck-item__info">
                    ...
                    <div class="ck-item__meta">{{ formatTime(item.date) }}<span v-if="item.location"> · {{ item.location }}</span></div>
                    <div class="ck-item__qty">{{ item.quantity }} × {{ Number(item.price).toFixed(2) }} DT</div>
                  </div>
                  <div class="ck-item__price">{{ (item.price * item.quantity).toFixed(2) }}<small> DT</small></div>
                </div>
              </div>
              ...
              <div class="ck-totals">
                <div class="ck-total-line"><span>Sous-total</span><span>{{ cart.totalPrice.toFixed(2) }} DT</span></div>
                <div class="ck-total-line"><span>Frais de service</span><span class="ck-free">✓ Offerts</span></div>
              </div>
              <div class="ck-grand">
                <span>Total à payer</span>
                <span class="ck-grand__amount">{{ cart.totalPrice.toFixed(2) }}<small> DT</small></span>
              </div>
              <button class="ck-pay-btn" @click="confirmPayment" :disabled="isProcessing || !isFormValid">
                ...
                {{ isStripeGateway ? 'Payer avec Stripe' : 'Confirmer le paiement' }}
              </button>
              <button class="ck-back-btn" @click="goToCart" :disabled="isProcessing">
              ...
              <p class="ck-notice">
                ...
                En confirmant, vous acceptez nos <a href="#" class="ck-notice__link">conditions d'utilisation</a>
              </p>
            </div>
          </aside>
        </div>
...
.ck { font-family: 'DM Sans', sans-serif; background: #fdf6f8; color: #3d2c35; min-height: 100vh; display: flex; flex-direction: column; }
.ck-nav { position: sticky; top: 0; z-index: 100; background: #fff; border-bottom: 1px solid #f5dce5; box-shadow: 0 1px 12px rgba(232, 64, 90, 0.05); padding: 0 40px; }
.ck-nav__inner { max-width: 1140px; margin: 0 auto; height: 66px; display: flex; align-items: center; justify-content: space-between; }
.ck-logo { display: flex; align-items: center; gap: 10px; cursor: pointer; text-decoration: none; }
.ck-logo__mark { width: 36px; height: 36px; border-radius: 10px; background: linear-gradient(135deg, #e8405a, #c4184a); display: flex; align-items: center; justify-content: center; color: #fff; font-family: 'Playfair Display', serif; font-weight: 800; font-size: 17px; box-shadow: 0 4px 14px rgba(232, 64, 90, 0.3); }
.ck-logo__name { font-family: 'Playfair Display', serif; font-weight: 800; font-size: 20px; color: #c4184a; letter-spacing: -0.3px; }
.ck-nav__secure { display: flex; align-items: center; gap: 7px; background: #f0fdf6; color: #059669; padding: 7px 14px; border-radius: 100px; font-size: 12px; font-weight: 600; border: 1px solid #d1fae5; }
.ck-stepper { background: #fff; padding: 20px 40px; border-bottom: 1px solid #f5dce5; }
.ck-steps { display: flex; align-items: center; justify-content: center; max-width: 460px; margin: 0 auto; }
.ck-step { display: flex; flex-direction: column; align-items: center; gap: 6px; }
.ck-step__bubble { width: 34px; height: 34px; border-radius: 50%; background: #f5e8ed; color: #c5a8b5; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 700; transition: all 0.3s; }
.ck-step__lbl { font-size: 10px; font-weight: 700; color: #c5a8b5; text-transform: uppercase; letter-spacing: 0.8px; }
.ck-step--active .ck-step__bubble { background: linear-gradient(135deg, #e8405a, #c4184a); color: #fff; box-shadow: 0 4px 14px rgba(232, 64, 90, 0.4); transform: scale(1.1); }
.ck-step--active .ck-step__lbl { color: #e8405a; }
.ck-step--done .ck-step__bubble { background: linear-gradient(135deg, #34d399, #059669); color: #fff; box-shadow: 0 3px 10px rgba(5, 150, 105, 0.3); }
.ck-step--done .ck-step__lbl { color: #059669; }
.ck-step__line { flex: 1; max-width: 90px; height: 2px; background: #f5dce5; margin: 0 10px 20px; border-radius: 2px; transition: background 0.4s; }
...
.ck-loader { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 16px; color: #b08090; font-size: 14px; }
.ck-loader__ring { width: 40px; height: 40px; border: 3px solid #f5dce5; border-top-color: #e8405a; border-radius: 50%; animation: spin 0.7s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
.ck-processing { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 20px; text-align: center; padding: 40px; position: relative; overflow: hidden; background: linear-gradient(160deg, #fdf6f8 0%, #fff0f4 50%, #fde8ef 100%); }
.ck-processing__rings { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; pointer-events: none; }
.ck-ring { position: absolute; border-radius: 50%; border: 1.5px solid rgba(232, 64, 90, 0.12); }
...
@keyframes ringPulse { 0% { transform: scale(0.8); opacity: 0.6; } 100% { transform: scale(1.2); opacity: 0; } }
.ck-processing__circle { width: 96px; height: 96px; border-radius: 50%; background: linear-gradient(135deg, #e8405a, #c4184a); display: flex; align-items: center; justify-content: center; box-shadow: 0 12px 36px rgba(232, 64, 90, 0.4); position: relative; z-index: 1; }
.ck-processing__circle--ok { background: linear-gradient(135deg, #34d399, #059669); box-shadow: 0 12px 36px rgba(5, 150, 105, 0.4); animation: popIn 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
@keyframes popIn { 0% { transform: scale(0); opacity: 0; } 70% { transform: scale(1.15); } 100% { transform: scale(1); opacity: 1; } }
.ck-processing__spinner { width: 38px; height: 38px; border: 3px solid rgba(255,255,255,0.3); border-top-color: #fff; border-radius: 50%; animation: spin 0.7s linear infinite; }
.ck-processing__title { font-family: 'Playfair Display', serif; font-size: 28px; font-weight: 800; color: #3d2c35; position: relative; z-index: 1; }
.ck-processing__sub { font-size: 14px; color: #b08090; position: relative; z-index: 1; }
.ck-psteps { display: flex; align-items: center; background: #fff; border: 1.5px solid #f5dce5; border-radius: 16px; padding: 16px 24px; box-shadow: 0 4px 20px rgba(232, 64, 90, 0.07); position: relative; z-index: 1; }
.ck-pstep { display: flex; flex-direction: column; align-items: center; gap: 6px; min-width: 80px; }
.ck-pstep__icon { width: 42px; height: 42px; border-radius: 12px; background: #f5e8ed; display: flex; align-items: center; justify-content: center; font-size: 19px; transition: all 0.3s; }
.ck-pstep--active .ck-pstep__icon { background: linear-gradient(135deg, #e8405a, #c4184a); animation: pulsate 0.9s ease-in-out infinite alternate; }
.ck-pstep--done .ck-pstep__icon { background: linear-gradient(135deg, #34d399, #059669); }
@keyframes pulsate { 0% { transform: scale(1); box-shadow: 0 0 0 0 rgba(232,64,90,0.4); } 100% { transform: scale(1.06); box-shadow: 0 0 0 8px rgba(232,64,90,0); } }
...
.ck-grid { display: grid; grid-template-columns: 1fr 360px; gap: 24px; align-items: flex-start; }
.ck-error { display: flex; align-items: flex-start; gap: 10px; background: #fef2f2; color: #b91c1c; border: 1.5px solid #fecaca; border-radius: 12px; padding: 13px 16px; font-size: 13px; font-weight: 500; margin-bottom: 20px; }
.ck-left { display: flex; flex-direction: column; gap: 18px; }
.ck-page-header { padding-bottom: 4px; }
.ck-page-header__title { font-family: 'Playfair Display', serif; font-size: 26px; font-weight: 800; color: #3d2c35; letter-spacing: -0.5px; margin-bottom: 5px; }
.ck-page-header__sub { font-size: 14px; color: #b08090; }
.ck-section { background: #fff; border-radius: 20px; border: 1.5px solid #f5dce5; overflow: hidden; box-shadow: 0 2px 16px rgba(232, 64, 90, 0.05); }
.ck-section__head { display: flex; align-items: center; gap: 11px; padding: 18px 24px; border-bottom: 1.5px solid #f5e8ed; background: linear-gradient(135deg, #fffbfc, #fef0f4); }
.ck-section__icon { width: 32px; height: 32px; border-radius: 9px; background: #fff5f7; border: 1px solid #ffd6de; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.ck-section__title { flex: 1; font-size: 14px; font-weight: 700; color: #3d2c35; letter-spacing: 0.1px; }
.ck-section__body { padding: 22px 24px; display: flex; flex-direction: column; gap: 14px; }
.ck-row { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
.ck-field { display: flex; flex-direction: column; gap: 6px; }
.ck-field label { font-size: 12px; font-weight: 700; color: #b08090; text-transform: uppercase; letter-spacing: 0.5px; display: flex; align-items: center; gap: 4px; }
.ck-req { color: #e8405a; }
.ck-cvv-tip { width: 16px; height: 16px; background: #f5e8ed; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; font-size: 10px; color: #c4184a; cursor: help; font-weight: 800; border: 1px solid #ffd6de; }
.ck-field input { padding: 12px 15px; border: 1.5px solid #f0e4e9; border-radius: 12px; font-size: 14px; font-family: 'DM Sans', sans-serif; color: #3d2c35; background: #fdfafc; outline: none; transition: all 0.2s; width: 100%; }
.ck-field input:focus { border-color: #e8405a; background: #fff; box-shadow: 0 0 0 4px rgba(232, 64, 90, 0.08); }
...
.ck-input-icon svg { position: absolute; left: 13px; top: 50%; transform: translateY(-50%); pointer-events: none; }
.ck-input-icon input { padding-left: 38px; }
.ck-input-suffix { position: relative; }
.ck-input-suffix input { padding-right: 44px; }
.ck-suffix-icon { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); }
...
.ck-gw { display: flex; align-items: center; gap: 12px; padding: 13px 16px; border: 1.5px solid #f0e4e9; border-radius: 13px; cursor: pointer; background: #fdfafc; transition: all 0.2s; }
.ck-gw:hover { border-color: #ffb3c1; background: #fff8f9; transform: translateY(-1px); }
.ck-gw--active { border-color: #e8405a; background: #fff5f7; box-shadow: 0 0 0 3px rgba(232, 64, 90, 0.08); }
...
.ck-gw__radio { width: 18px; height: 18px; border-radius: 50%; border: 2px solid #f0d0dc; display: flex; align-items: center; justify-content: center; transition: border-color 0.2s; }
.ck-gw--active .ck-gw__radio { border-color: #e8405a; }
.ck-gw__dot { width: 9px; height: 9px; border-radius: 50%; background: #e8405a; }
.ck-stripe-info { display: flex; align-items: center; gap: 10px; background: #f0f0ff; border: 1.5px solid #c7d2fe; border-radius: 11px; padding: 12px 16px; font-size: 13px; color: #4338ca; }
.ck-card-badges { display: flex; gap: 5px; }
.ck-cb { padding: 3px 8px; border-radius: 5px; font-size: 9px; font-weight: 800; letter-spacing: 0.5px; }
...
.ck-vcard { margin: 0 24px 4px; border-radius: 18px; padding: 24px 26px 22px; background: linear-gradient(135deg, #7c3aed 0%, #4f46e5 50%, #3b0764 100%); position: relative; overflow: hidden; box-shadow: 0 14px 40px rgba(79, 70, 229, 0.35); min-height: 188px; transition: background 0.4s; }
...
.ck-vcard__shine { position: absolute; inset: 0; background: linear-gradient(105deg, transparent 40%, rgba(255,255,255,0.07) 50%, transparent 60%); animation: cardShine 3.5s infinite; }
@keyframes cardShine { 0% { transform: translateX(-120%); } 100% { transform: translateX(120%); } }
.ck-vcard::before { content: ''; position: absolute; top: -50px; right: -50px; width: 200px; height: 200px; border-radius: 50%; background: rgba(255,255,255,0.07); }
.ck-vcard::after { content: ''; position: absolute; bottom: -40px; left: -20px; width: 160px; height: 160px; border-radius: 50%; background: rgba(255,255,255,0.04); }
.ck-vcard__top { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px; position: relative; z-index: 1; }
.ck-vcard__chip { width: 38px; height: 28px; background: linear-gradient(135deg, #ffe082, #f59e0b); border-radius: 5px; position: relative; }
.ck-vcard__chip-line { position: absolute; top: 50%; left: 0; right: 0; height: 1px; background: rgba(120, 80, 0, 0.35); transform: translateY(-50%); }
.ck-vcard__chip-col { position: absolute; left: 50%; top: 0; bottom: 0; width: 1px; background: rgba(120, 80, 0, 0.35); }
...
.ck-vcard__number { font-family: 'Space Mono', monospace; font-size: 18px; font-weight: 400; color: rgba(255,255,255,0.95); letter-spacing: 2px; margin-bottom: 18px; position: relative; z-index: 1; }
.ck-vcard__footer { display: flex; justify-content: space-between; align-items: flex-end; position: relative; z-index: 1; }
.ck-vcard__col { display: flex; flex-direction: column; gap: 3px; }
.ck-vcard__col--right { text-align: right; }
.ck-vcard__label { font-size: 9px; text-transform: uppercase; letter-spacing: 1.2px; color: rgba(255,255,255,0.5); }
.ck-vcard__value { font-family: 'Space Mono', monospace; font-size: 12px; font-weight: 700; color: rgba(255,255,255,0.95); text-transform: uppercase; letter-spacing: 0.5px; }
.ck-security { display: flex; gap: 10px; flex-wrap: wrap; padding: 12px 14px; background: #f0fdf6; border-radius: 11px; border: 1px solid #d1fae5; }
.ck-security__item { display: flex; align-items: center; gap: 5px; font-size: 11px; font-weight: 600; color: #059669; }
.ck-right { position: sticky; top: 86px; }
.ck-summary { background: #fff; border-radius: 22px; border: 1.5px solid #f5dce5; padding: 24px; box-shadow: 0 6px 28px rgba(232, 64, 90, 0.09); }
.ck-summary__title { font-family: 'Playfair Display', serif; font-size: 17px; font-weight: 800; color: #3d2c35; margin-bottom: 2px; }
...
.ck-item { display: grid; grid-template-columns: 44px 1fr auto; gap: 10px; align-items: center; padding: 11px 12px; background: #fdf6f8; border-radius: 12px; border: 1px solid #f5e8ed; }
.ck-item__date { background: #fff; border: 1px solid #ffd6de; border-radius: 8px; padding: 5px 4px; display: flex; flex-direction: column; align-items: center; }
...
.ck-item__title { font-size: 12px; font-weight: 700; color: #3d2c35; margin-bottom: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.ck-gw-pill { display: inline-flex; align-items: center; gap: 6px; background: #eff6ff; color: #2563eb; border: 1px solid #bfdbfe; padding: 4px 11px; border-radius: 20px; font-size: 11px; font-weight: 500; margin-bottom: 10px; }
.ck-sep { height: 1px; background: #f5e8ed; margin: 10px 0; }
.ck-totals { display: flex; flex-direction: column; gap: 7px; margin-bottom: 8px; }
.ck-total-line { display: flex; justify-content: space-between; align-items: center; font-size: 13px; color: #7d5a68; }
.ck-free { background: #f0fdf6; color: #059669; font-size: 11px; font-weight: 700; padding: 3px 9px; border-radius: 20px; border: 1px solid #bbf7d0; }
.ck-grand { display: flex; justify-content: space-between; align-items: center; padding: 13px 0 16px; border-top: 1.5px dashed #f5dce5; margin-top: 4px; }
.ck-grand span:first-child { font-size: 14px; font-weight: 700; color: #3d2c35; }
.ck-grand__amount { font-family: 'Playfair Display', serif; font-size: 26px; font-weight: 800; color: #e8405a; letter-spacing: -0.5px; }
.ck-grand__amount small { font-size: 13px; font-weight: 600; color: #b08090; }
.ck-pay-btn { width: 100%; display: flex; align-items: center; justify-content: center; gap: 9px; padding: 15px; background: linear-gradient(135deg, #e8405a, #c4184a); border: none; border-radius: 13px; color: #fff; font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 700; cursor: pointer; transition: all 0.25s; box-shadow: 0 6px 20px rgba(232, 64, 90, 0.35); margin-bottom: 10px; letter-spacing: 0.2px; }
.ck-pay-btn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 10px 28px rgba(232, 64, 90, 0.45); }
.ck-pay-btn:active:not(:disabled) { transform: translateY(0); }
.ck-pay-btn:disabled { opacity: 0.45; cursor: not-allowed; transform: none; }
.ck-back-btn { width: 100%; display: flex; align-items: center; justify-content: center; gap: 6px; padding: 11px; background: transparent; border: 1.5px solid #f0d0dc; border-radius: 13px; color: #e8405a; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; margin-bottom: 12px; }
.ck-back-btn:hover:not(:disabled) { background: #fff5f7; border-color: #e8405a; }
.ck-back-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.ck-notice { display: flex; align-items: flex-start; gap: 6px; font-size: 11px; color: #b08090; line-height: 1.5; }
.ck-notice__link { color: #e8405a; text-decoration: none; font-weight: 600; }
.ck-notice__link:hover { text-decoration: underline; }
.ck-footer { background: #fdf0f4; border-top: 1px solid #f5dce5; padding: 18px 40px; display: flex; justify-content: space-between; font-size: 12px; color: #c5a8b5; margin-top: auto; }
...
.ck-ticket-hero { display: flex; align-items: center; gap: 18px; background: linear-gradient(135deg, #fff0f4, #fde8ef); border: 1.5px solid #ffd6de; border-radius: 20px; padding: 22px 26px; margin-bottom: 28px; box-shadow: 0 4px 20px rgba(232,64,90,0.08); }
.ck-ticket-hero__icon { width: 60px; height: 60px; border-radius: 50%; background: linear-gradient(135deg, #34d399, #059669); display: flex; align-items: center; justify-content: center; flex-shrink: 0; box-shadow: 0 6px 20px rgba(5,150,105,0.3); }
.ck-ticket-hero__title { font-family: 'Playfair Display', serif; font-size: 22px; font-weight: 800; color: #3d2c35; margin-bottom: 4px; }
...
.ck-ticket { background: #fff; border-radius: 20px; overflow: hidden; box-shadow: 0 8px 36px rgba(232,64,90,0.13), 0 2px 8px rgba(0,0,0,0.04); border: 1px solid #f5dce5; }
.ck-ticket__body { display: grid; grid-template-columns: 1fr 170px; min-height: 230px; }
.ck-ticket__left { padding: 26px 26px 22px; display: flex; flex-direction: column; gap: 0; }
.ck-ticket__event { display: flex; gap: 16px; align-items: flex-start; margin-bottom: 18px; }
.ck-ticket__date-badge { background: linear-gradient(135deg, #e8405a, #c4184a); border-radius: 12px; padding: 10px 13px; display: flex; flex-direction: column; align-items: center; flex-shrink: 0; box-shadow: 0 5px 15px rgba(232,64,90,0.3); }
...
.ck-ticket__event-label { font-size: 9px; font-weight: 700; color: #e8405a; letter-spacing: 1.5px; text-transform: uppercase; margin-bottom: 4px; }
.ck-ticket__event-name { font-family: 'Playfair Display', serif; font-size: 17px; font-weight: 800; color: #3d2c35; line-height: 1.25; margin-bottom: 7px; }
.ck-ticket__event-meta { display: flex; align-items: center; gap: 5px; font-size: 11px; color: #b08090; margin-bottom: 3px; }
.ck-ticket__tear { display: flex; align-items: center; margin: 12px 0; }
.ck-ticket__tear-notch { width: 18px; height: 18px; border-radius: 50%; background: #fdf6f8; border: 1px solid #f5dce5; flex-shrink: 0; }
...
.ck-ticket__details { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; }
.ck-ticket__detail-lbl { font-size: 9px; font-weight: 700; color: #c5a8b5; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 3px; }
.ck-ticket__detail-val { font-size: 12px; font-weight: 700; color: #3d2c35; }
.ck-ticket__detail-val--pink { color: #e8405a; }
.ck-ticket__qr { background: linear-gradient(160deg, #fff8fa, #fef0f4); border-left: 2px dashed #f5dce5; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 22px 16px; gap: 9px; }
.ck-ticket__qr-box { background: #fff; border-radius: 12px; padding: 8px; box-shadow: 0 4px 14px rgba(232,64,90,0.1); border: 1px solid #f5dce5; }
.ck-ticket__qr-img { width: 110px; height: 110px; display: block; }
.ck-ticket__qr-label { font-size: 9px; font-weight: 600; color: #c5a8b5; text-transform: uppercase; letter-spacing: 0.8px; text-align: center; }
...
.ck-ticket__barcode { background: linear-gradient(135deg, #5d3a48, #3d2c35); padding: 12px 24px; display: flex; flex-direction: column; align-items: center; gap: 5px; }
...
.ck-ticket__barcode-txt { font-family: 'Space Mono', monospace; font-size: 9px; color: rgba(255,255,255,0.45); letter-spacing: 2px; text-transform: uppercase; }
.ck-ticket-actions { display: flex; gap: 12px; justify-content: center; margin-bottom: 20px; flex-wrap: wrap; }
.ck-ticket-btn { display: flex; align-items: center; gap: 9px; padding: 13px 24px; border-radius: 13px; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 700; cursor: pointer; transition: all 0.25s; border: none; }
.ck-ticket-btn--primary { background: linear-gradient(135deg, #e8405a, #c4184a); color: #fff; box-shadow: 0 6px 20px rgba(232,64,90,0.3); }
.ck-ticket-btn--primary:hover { transform: translateY(-2px); box-shadow: 0 10px 28px rgba(232,64,90,0.4); }
.ck-ticket-btn--dashboard { background: linear-gradient(135deg, #7c3aed, #4f46e5); color: #fff; box-shadow: 0 6px 20px rgba(79,70,229,0.3); }
.ck-ticket-btn--dashboard:hover { transform: translateY(-2px); box-shadow: 0 10px 28px rgba(79,70,229,0.4); }
.ck-ticket-btn--outline { background: #fff; color: #e8405a; border: 1.5px solid #f0d0dc !important; }
.ck-ticket-btn--outline:hover { background: #fff5f7; border-color: #e8405a !important; }
.ck-ticket-notice { display: flex; align-items: center; gap: 8px; justify-content: center; font-size: 12px; color: #b08090; text-align: center; }
@media print { .ck-nav, .ck-stepper, .ck-ticket-hero, .ck-ticket-actions, .ck-ticket-notice, .ck-footer { display: none !important; } .ck-ticket { box-shadow: none; border: 1px solid #ddd; } }
@media (max-width: 900px) { .ck-grid { grid-template-columns: 1fr; } .ck-right { position: static; } }
@media (max-width: 600px) { .ck-nav { padding: 0 16px; } .ck-wrap { padding: 0 14px; } .ck-row { grid-template-columns: 1fr; } .ck-footer { flex-direction: column; gap: 4px; align-items: center; } .ck-stepper { padding: 16px 14px; } .ck-ticket__body { grid-template-columns: 1fr; } .ck-ticket__qr { border-left: none; border-top: 2px dashed #f5dce5; flex-direction: row; justify-content: space-around; } .ck-ticket__details { grid-template-columns: repeat(2,1fr); } }
</style>