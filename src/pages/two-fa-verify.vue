...
async function handleVerify() {
  ...
  const result = await authStore.verify2FA(userId.value, otp.value)
  loading.value = false
  if (result.success) {
...
async function handleResend() {
  ...
  const result = await authStore.resend2FA(userId.value)
  if (result.success) {
  ...
  <div>
    ...
    <div class="lp">
      ...
      <aside class="hero">
        ...
        <h1 class="hero-h1">Votre sécurité,<br><em>notre priorité</em></h1>
        <p class="hero-p">La vérification en deux étapes protège votre compte contre les accès non autorisés.</p>
      ...
      <main class="card">
        ...
        <p class="card-eyebrow">Authentification à deux facteurs</p>
