  ...
  <VLocaleProvider :rtl="configStore.isAppRTL">
    <VApp :style="`--v-global-theme-primary: ${hexToRgb(global.current.value.colors.primary)}`">
      ...
      <div class="atp-wrap">
        <transition name="atp-panel">
          <div v-if="showPalette" class="atp-panel">
            ...
            <div class="atp-section">
              ...
              <div class="atp-names">
                <span v-for="t in themes" :key="t.id" :class="{ active: currentTheme.id === t.id }">
                  {{ t.label }}
                </span>
              </div>
            </div>
            ...
            <div class="atp-section">
              ...
              <div class="atp-mode" @click="toggleDark">
                <div class="atp-mode-opt" :class="{ active: !isDark }">☀️ Clair</div>
                <div class="atp-mode-opt" :class="{ active: isDark }">🌙 Sombre</div>
              </div>
            </div>
        ...
        <button class="atp-fab" @click.stop="showPalette = !showPalette" title="Personnaliser">
          <transition name="atp-icon" mode="out-in">
            ...
            <svg v-else key="p" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
<style>
/* Variables globales — héritées par toutes les pages */
:root {
  --accent:      #D4215A;
  --accent-lt:   #FF4D7A;
  --accent-dk:   #9B1040;
...
.atp-fab:hover { transform: scale(1.1) rotate(12deg); }
...
.atp-label {
  ...
  letter-spacing: .1em; color: #7A5060;
  margin-bottom: 14px;
}
html.dark .atp-label { color: #9a7080; }

.atp-sep { height: 1px; background: #F0DCE4; margin: 18px 0; }
html.dark .atp-sep { background: #2d1520; }

.atp-swatches { display: flex; gap: 10px; justify-content: space-between; margin-bottom: 8px; }
.atp-swatch {
...
.atp-swatch.active { border-color: #fff; box-shadow: 0 0 0 3px var(--sw); transform: scale(1.18); }
html.dark .atp-swatch.active { border-color: #1a0f13; }

.atp-names { display: flex; justify-content: space-between; }
.atp-names span { font-size: 9px; color: #9a7080; text-align: center; flex: 1; transition: color .2s; }
.atp-names span.active { color: var(--accent, #D4215A); font-weight: 700; }

.atp-mode {
  display: flex; background: #FDF8F5;
  border-radius: 12px; padding: 4px;
  border: 1.5px solid #F0DCE4;
  cursor: pointer; transition: border-color .2s;
}
html.dark .atp-mode { background: #120810; border-color: #2d1520; }
.atp-mode:hover { border-color: var(--accent, #D4215A); }
.atp-mode-opt {
  flex: 1; padding: 9px 4px; border-radius: 9px;
  font-size: 12px; font-weight: 600; text-align: center;
  color: #7A5060; transition: all .25s;
}
html.dark .atp-mode-opt { color: #9a7080; }
.atp-mode-opt.active {
  background: #fff;
  color: var(--accent, #D4215A);
  box-shadow: 0 2px 10px rgba(var(--accent-rgb, 212,33,90), .18);
}
html.dark .atp-mode-opt.active { background: #1a0f13; }
